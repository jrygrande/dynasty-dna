...
runExperiment({
  ...
  run: async (ctx) => {
    ...
    allCases.sort((a, b) => b.scoreDiff - a.scoreDiff);
    const top20 = allCases.slice(0, 20);

    if (top20.length === 0) {
      ctx.log("  No trade cases found.");
    } else {
      ctx.log(`\n  Top ${top20.length} trades with largest score difference:\n`);
    ...
    // Summary stats
    const allDiffs = allCases.map((c) => c.scoreDiff);
    const avgDiff =
      allDiffs.length > 0
        ? Math.round((allDiffs.reduce((a, b) => a + b, 0) / allDiffs.length) * 10) / 10
        : 0;
    const bigSwings = allCases.filter((c) => c.scoreDiff > 10).length;
    const bigSwingPct = Math.round((bigSwings / Math.max(allCases.length, 1)) * 100);

    ctx.log(`\n  Summary: ${allCases.length} trade sides analyzed`);
    ctx.log(`  Average score difference: ${avgDiff}`);
