...
gh label create "phase:4" --color "bfdadc" --description "Phase 4: Grading Engine" --repo "$REPO" --force
gh label create "phase:5" --color "bfdadc" --description "Phase 5: Manager Analytics" --repo "$REPO" --force
gh label create "phase:6" --color "bfdadc" --description "Phase 6: Exploration & Polish" --repo "$REPO" --force
...
# ─── SHIPPED (Phase 2: Data Foundation) ───
...
# ─── SHIPPED (Phase 3: NFL Data & Player Insights) ───
...
# ─── SHIPPED (Phase 4: Grading Engine) ───

gh issue create --repo "$REPO" --title "Lineup Optimization Score" \
  --label "roadmap,type:feature,status:shipped,phase:4,priority:p0" \
  --body "$(cat <<'BODY'
### Problem Statement
...
gh issue create --repo "$REPO" --title "Draft Grading (Pick Value vs. Performance)" \
  --label "roadmap,type:feature,status:shipped,phase:4,priority:p1" \
  --body "$(cat <<'BODY'
### Problem Statement
...
- Adaptive scaling based on elapsed games since draft
...
# ─── PLANNED (Phase 5: Manager Analytics) ───

gh issue create --repo "$REPO" --title "Waiver/FA Grading" \
  --label "roadmap,type:feature,status:planned,phase:5,priority:p0" \
  --body "$(cat <<'BODY'
### Problem Statement
Managers make waiver claims and free-agent pickups but have no way to evaluate those decisions over time.

### Hypothesis
Grading waiver/FA moves using post-acquisition production, drop opportunity cost, and FAAB efficiency will help managers improve their roster construction through the season.

### Approach
- Track production of acquired players post-pickup
- Calculate drop opportunity cost (production of dropped player elsewhere)
- FAAB efficiency score for leagues using FAAB
- Grade scale: A+ to F with context labels

### Success Metrics
- Engagement with waiver grade detail
- Correlation between waiver grade awareness and future waiver success
BODY
)" && echo "Created: Waiver/FA Grading"
...
gh issue create --repo "$REPO" --title "Manager Profile Page" \
  --label "roadmap,type:feature,status:planned,phase:5,priority:p1" \
  --body "$(cat <<'BODY'
### Problem Statement
Managers need a dedicated page that aggregates all their analytics into a single profile view.

### Hypothesis
A unified manager profile at /league/[familyId]/manager/[userId] with radar chart, grade summary, and badges will become the most-visited page per session.

### Approach
- Dedicated route: /league/[familyId]/manager/[userId]
- Radar chart showing all grading dimensions
- Grade cards for each pillar (lineup, trade, draft, waiver)
- Achievement badges for notable patterns
- Season-over-season comparison

### Success Metrics
- Page views per session (target: most-visited page)
- Time on page
- Share rate of manager profiles
BODY
)" && echo "Created: Manager Profile Page"

gh issue create --repo "$REPO" --title "Lineup Efficiency UI Card" \
  --label "roadmap,type:feature,status:planned,phase:5,priority:p2" \
  --body "$(cat <<'BODY'
### Problem Statement
Lineup efficiency data exists in the backend but is not surfaced prominently on the league overview page.

### Hypothesis
An expandable lineup efficiency card on the league overview showing weekly detail will increase awareness of lineup decisions without requiring navigation to a separate page.

### Approach
- Compact card on league overview with season-level grade
- Expandable weekly detail showing optimal vs actual
- Highlight worst weeks for quick identification

### Success Metrics
- Card expansion rate
- Reduction in navigation to standalone lineup page
BODY
)" && echo "Created: Lineup Efficiency UI Card"

# ─── EXPLORING (Phase 6: Exploration & Polish) ───

gh issue create --repo "$REPO" --title "Automated Data Refresh / Cron Jobs" \
  --label "roadmap,type:feature,status:planned,phase:6,priority:p1" \
  --body "$(cat <<'BODY'
### Problem Statement
Data currently refreshes only on user-triggered sync. Managers expect fresh data without manual intervention.

### Hypothesis
Automated background refresh via Vercel Cron will reduce stale data complaints and improve trust in analytics accuracy.

### Approach
- Vercel Cron jobs for weekly data refresh
- Stale data indicator in UI when data is older than threshold
- Incremental sync to minimize API calls

### Success Metrics
- % of page loads with fresh data (< 24h old)
- Reduction in manual sync triggers
BODY
)" && echo "Created: Automated Data Refresh"

gh issue create --repo "$REPO" --title "Asset Graph Browser" \
  --label "roadmap,type:feature,status:exploring,phase:6,priority:p2" \
  --body "$(cat <<'BODY'
### Problem Statement
...
# ─── PLANNED (Phase 6: Exploration & Polish, lower priority) ───
