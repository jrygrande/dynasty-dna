...
async function linkSelectionsToPicksV3(): Promise<void> {
  console.log('🔗 Starting final draft selections to picks linking...');
  ...
  try {
    ...
    for (const selection of selections) {
      try {
        ...
        const draftSlot = selection.draftSlot; // This is the slot within the round
        const rosterId = selection.rosterId; // This is the Sleeper roster ID

        console.log(`Processing ${season} R${round}P${selection.pickNumber}: ${selection.player.fullName} (roster ${rosterId}, slot ${draftSlot})`);
        ...
        // 2. Same pickInRound (which corresponds to draftSlot)
        const draftPick = await prisma.draftPick.findFirst({
          where: {
            leagueId: league.id,
            season: season,
            round: round,
            pickInRound: draftSlot // This should match the selection's draftSlot
          }
        });
        ...
        if (draftPick) {
          ...
          linked++;
          console.log(`    ✅ Linked to slot ${draftSlot} (${manager.username})`);
        } else {
          console.warn(`    ❌ Could not find draft pick for ${season} R${round} slot ${draftSlot}`);
          notFound++;
        }
...
export { linkSelectionsToPicksV3 };